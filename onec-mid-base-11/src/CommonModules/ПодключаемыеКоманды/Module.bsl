...
Процедура ПриСозданииНаСервере(Форма, Знач ПараметрыРазмещения = Неопределено) Экспорт
	...
	// Добавление дополнительных элементов на формы элементов в документах
	Если ЭтоФормаОбъекта Тогда
		Если (СтрНачинаетсяС(ИмяФормы, "Документ.")) Тогда 
			Terabyte_ДобавлениеЭлементовНаФормыДокументов.ДобавитьЭлементыНаФормыДокументов(Форма, ИмяФормы);
		КонецЕсли;
	КонецЕсли;
	// Конец изменений Столповский Д.А. 03.01.2024
